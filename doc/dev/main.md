...
The _Arduino MKR WAN 1310_ handles all the data processing and communication via a public _LoRa_ wireless network. The firmware is configured to send a message each time the _Arduino_ is powered on. It includes the state of the pin _D7_, which represents the reed switch. This input pin can be changed in the `keep_alive.ino`:
...
For debugging purpose, the preprocessor flag `DEBUG`, which enables the `Serial.print()` log. It must be disabled, if there is no USB connection to the PC, because the command waits until the USB-serial communictaion is ready, which blocks the firmware completely. 
...
It is important to limit the _LoRa_ retry interval in `keep_alive.ino`, because there are phases, when the network resources are exhausted. The retry limit prevents wasting energy:
