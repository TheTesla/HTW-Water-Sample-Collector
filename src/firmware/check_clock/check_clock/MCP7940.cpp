...
bool MCP7940_Class::oscOK() const {
  /*!
      @brief  checks to see if the MCP7940 crystal is working
      @brief  Reads the status register check for osc failure
      @return Success status true if the crystal is working, otherwise false if off
   */
  return readRegisterBit(MCP7940_RTCWKDAY, MCP7940_OSCRUN);
}  // of method DeviceStatus
...
bool MCP7940_Class::deviceStart() const {
  ...
  clearRegisterBit(MCP7940_CONTROL, MCP7940_EXTOSC);  // Set the ST bit
